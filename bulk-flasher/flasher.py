...
def main():
    """
    Основна функція скрипта прошивки.

    Виконує такі кроки:
        1. Перевіряє наявність виконуваного файлу tftp-now.
        2. Визначає шлях до папки з прошивками (аргумент командного рядка або за замовчуванням).
        3. Створює тимчасову папку в /tmp для файлу tp_recovery.bin.
        4. Запускає TFTP-сервер один раз (через sudo) із коренем у тимчасовій папці.
        5. Для кожного вузла:
            - Копіює файл із *tftp-recovery*.bin як tp_recovery.bin.
            - Чекає на натискання Enter після завершення прошивки.
        6. Зупиняє сервер і видаляє тимчасову папку.

    Поведінка залежить від аргументів командного рядка:
        - Якщо передано один аргумент, він вважається шляхом до папки bin.
        - Якщо аргументів немає, використовується ../openwrt-generator/bin.

    Вимагає права sudo для запуску tftp-now на порту 69.
    """
